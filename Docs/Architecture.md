...
2. historical studies 與 internal documents。
3. confirmed-unused helper utilities。
4. 產品沒有 exposes 的資料檔與模組。

需要先檢查 build graph 的 candidates：

1. 目前產品沒有 exposes 的 extra modules
2. `Takao.xcodeproj` 仍引用的 helper apps
3. 仍由 runtime 或偏好設定間接載入的資料檔
4. 仍影響 signing、packaging 或 local install scripts 的 legacy helper
